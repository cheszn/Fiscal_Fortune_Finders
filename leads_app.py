...
try:
    leads_model = joblib.load(r"./models/xgboost_classifier.joblib")
...
def online_input():
    ...
    lead_origin = st.sidebar.selectbox("Lead Origin", ['API', 'Landing Page Submission', 'Lead Add Form', 'Lead Import',
                                                      'Quick Add Form'])
    lead_source = st.sidebar.selectbox("Lead Source", ['Drift', 'Organic Search', 'Direct Traffic', 'Google',
    ...
    total_time_spent_on_website = st.sidebar.number_input('Total Time Spent on Website (minutes)', step=1, format="%d")
    last_activity = st.sidebar.selectbox("Last Activity", ['Page Visited on Website', 'Email Opened', 'Unreachable',
                                                      'Converted to Lead', 'Drift Chat Conversation', 'Email Bounced',
                                                      'Email Link Clicked', 'Form Submitted on Website', 'Unsubscribed',
                                                      'Had a Phone Conversation', 'View in browser link Clicked',
                                                      'Approached upfront', 'SMS Sent', 'Visited Booth in Tradeshow',
                                                      'Resubscribed to emails', 'Email Received', 'Email Marked Spam'])
    lead_quality = st.sidebar.selectbox("Lead Quality", ['Low in Relevance', 'Might be', 'Not Sure', 'Worst',
                                                      'High in Relevance'])
    lead_profile = st.sidebar.selectbox("Lead Profile", ['Select', 'Potential Lead', 'Other Leads', 'Lateral Student',
                                                      'Dual Specialization Student', 'Student of SomeSchool'])
    asymmetrique_profile_score = st.sidebar.number_input('Asymmetrique Profile Score', min_value=0, max_value=200, value=0)
    ...
    if st.sidebar.button('Predict'):
        ...
        prediction = leads_model.predict(pd.DataFrame(user_input_data))[0]
